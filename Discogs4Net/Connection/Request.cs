...
namespace Discogs4Net.Connection
{
    internal class Request
    {
        ...
        protected internal String AppCode
        {
            get
            {
                return appCode;
            ...
            private set
            {
                ...
                appCode = String.Format("{0}/{1}; {2}", Library.Default.LibraryName, fvi.ProductVersion, value);
            }
        }

        public Request(string appCode)
        {
            if (String.IsNullOrEmpty(appCode))
                throw new ArgumentException("An application identifier must be provided before instantiating a request object", "appCode");

            AppCode = appCode;
        }

        private HttpWebRequest CreateRequest(string url, string bodyData)
        {
            ...
            using (var stream = request.GetRequestStream())
            {
                ...
                var bytes = new UTF8Encoding().GetBytes(bodyData);
        ...
        private HttpWebRequest CreateRequest(string url)
        {
            if (string.IsNullOrEmpty(url))
                throw new ArgumentException("URL is mandatory", "url");
        ...
        {
            return ProcessResponse(request.GetResponse());
        }
        ...
        public string Get(string url)
        {
            ...
            try
            {
                ...
                var request = CreateRequest(url);
                request.Method = "GET";
                responseString = ProcessResponse(request.GetResponse());
            }
            catch (Exception)
            {
                throw;
        ...
        public string Post(string url, string data)
        {
            ...
            try
            {
                ...
                request.Method = "POST";
                response = ProcessResponse(request.GetResponse());
            }
            catch (Exception)
            {
                throw;
